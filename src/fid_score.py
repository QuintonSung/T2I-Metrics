...
from parser_config.get_parser import get_basic_parser, get_fid_value_parser, merged_parser
...
def compute_is_of_path(path, model, batch_size, dims, device,
...
def cal_fid_value(paths, batch_size, device, dims, num_workers=1):
    ...
    # print('Inception Score is{} +/- {}'.format(IS, IS_std))
...
def main(args):
    ...
    if args.save_stats:
        save_fid_stats([args.path1, args.path2], args.batch_size, device, args.fid_dims, num_workers)
        return
...
if __name__ == '__main__':
    ...
    args = merged_parser(*[basic_parser, fid_value_parser])
